...
import { setQaStatus, toggleQaStep, generateAiQaSteps, setAiStepTicks } from "./actions";
...
export function QaBoard({
  ...
  // An id rather than the item itself. Holding the object froze the dialog at
  // the moment it opened, so a server action that revalidated the page (adding
  // generated QA steps, say) left the open dialog showing the old data until the
  // reviewer closed it and looked again.
...
/**
 * Tickable generated steps.
 *
 * Deliberately a separate component from <QaSteps>, and not a reuse of it. That
 * one rewrites the pull request description on GitHub, matching each line
 * against text that really exists there. These steps exist only in our database,
 * so there is nothing on GitHub to match and nowhere to write back to; ticks are
 * stored locally against the AiResult row instead.
 *
 * Same batching behaviour though, for the same reason: a reviewer working down a
 * checklist should cause one write, not one per box.
 */
function AiStepChecklist({
  projectId,
  itemId,
  steps,
  ticked,
  ...
  const [local, setLocal] = useState<Set<number>>(() => new Set(ticked));
  const [error, setError] = useState<string | null>(null);

  // Re-sync when the server sends a different checklist, which happens on a
  // regenerate. Without this the reviewer would see new steps carrying the old
  // ticks, which is worse than no ticks at all.
  const key = useMemo(() => `${steps.length}:${ticked.join(",")}`, [steps.length, ticked]);
  const lastKey = useRef(key);
  useEffect(() => {
    if (lastKey.current !== key) {
      lastKey.current = key;
      setLocal(new Set(ticked));
    }
  }, [key, ticked]);
  ...
  const flush = useCallback(async () => {
    ...
    try {
      const res = await setAiStepTicks({
        projectId,
        itemId,
        ticked: [...localRef.current],
      });
      if (!res.ok) setError(res.error);
    } catch {
      // A failed save costs a tick, not the reviewer's work: the steps and the
      // verdict buttons are unaffected, and the next tick retries.
      setError("Could not save that. Try again.");
    }
  }, [projectId, itemId]);

  // Closing the dialog unmounts this, which is every way out of it.
  const flushRef = useRef(flush);
  flushRef.current = flush;
  ...
  const done = steps.filter((_, i) => local.has(i)).length;
      ...
      <ul className="space-y-1.5">
        {steps.map((step, i) => (
          <li key={i} className="flex items-start gap-2 text-sm">
            ...
            />
            <span className={local.has(i) ? "text-muted-foreground line-through" : undefined}>
              {step}
            </span>
          </li>
        ))}
